...
func OpenDBFile(dbPath string, pageSize int) (*DBFile, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("OpenFileFailed %s, err:%v", dbPath, err)
	}
	defer fp.Close()

	fileInfo, err := fp.Stat()
	if err != nil {
		return nil, fmt.Errorf("StatFileFailed %s, err:%v", dbPath, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ReadFileFailed %s,err:%+v", dbPath, err)
	}
	if n != pageSize {
		return nil, fmt.Errorf("incompleteRead %s, read %d bytes, expected %d", dbPath, n, pageSize)
	}

	if bytes.Equal(buffer[:len(SQLiteHeader)-1], []byte(SQLiteHeader[:len(SQLiteHeader)-1])) {
		return nil, fmt.Errorf("database file is already decrypted")
	}
...
func DecryptPage(pageBuf []byte, encKey []byte, macKey []byte, pageNum int64, hashFunc func() hash.Hash, hmacSize int, reserve int, pageSize int) ([]byte, error) {
	...
	if !bytes.Equal(hashMac, pageBuf[hashMacStartOffset:hashMacEndOffset]) {
		return nil, fmt.Errorf("StatFileFailed")
	}
